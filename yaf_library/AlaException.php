...
class AlaException extends Exception {
	...
	static public function log($exception, $path) {
		$time = time();
		$url = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
		$trace = json_encode(self::getTraces($exception, $format = 'plain'));
		$msg = $exception->getMessage();
		$host = $_SERVER["HTTP_HOST"];
		$host_ip = $_SERVER["SERVER_ADDR"];
		$remote_ip = $_SERVER['REMOTE_ADDR'];
		$client_ip = Util_Tool::getRealIP();
		@$format = "500	{$time} {$_SERVER['HTTP_REFERER']}	$url	$trace	$msg	$host	$host_ip	$remote_ip	$client_ip\n";
		
		$formatter = new Zend_Log_Formatter_Simple($format);
		$writer = new Zend_Log_Writer_Stream($path);
		$writer->setFormatter($formatter);
		$logger = new Zend_Log($writer);
		$logger->log("", Zend_Log::CRIT);
