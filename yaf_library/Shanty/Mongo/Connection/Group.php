...
class Shanty_Mongo_Connection_Group
{
	...
	public function __construct(array $connectionOptions = null)
	{
		...
		if (!is_null($connectionOptions)) {
			...
			$config = Yaf_Application::app ()->getConfig (); //Yaf_Registry::get('default_config');
			if (empty($config->mongodb) || empty($config->mongodb->servers)) {
				throw new Shanty_Mongo_Exception("No mongodb config!");
			}

			$this->addConnections($config->mongodb->servers->toArray());
			...
			foreach ($servers as $type => $server) {
				...
				foreach ($server as $info) {
					$conn_str = "{$info['host']}:{$info['port']}";
					$conn = new Shanty_Mongo_Connection($conn_str);

					if (array_key_exists('weight', $info)) {
						$weight = $info['weight'];
					}
					$this->$func($conn, $weight);
				}
			}
			*/
		}
	...
	public function addConnections($connectionOptions)
	{
		...
		if (array_key_exists('master', $connectionOptions)) 
			$masters[] = $connectionOptions['master']; // single master
		...
		elseif (array_key_exists('hosts', $connectionOptions)) {
			$masters[] = $connectionOptions; // one server
		}
	...
	public function formatHostString(array $hostOptions = array())
	{
		...
		if (isset($hostOptions['host']) && !is_null($hostOptions['host'])) 
			$hostString .= $hostOptions['host'];
		else 
