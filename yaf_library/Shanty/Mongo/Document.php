	...
	public function setProperty($property, $value)
	{
		...
		if (!is_null($value) && !$validators->isValid($value)) {
			...
			foreach($messages as $k=>$v){
				$messages[$k]=$property.' is invalid. '.$v;
			}
    ...
    */
	...
	public function export($skipRequired = false)
	{
		...
		foreach ($this->_data as $property => $value) {
			...
			if ($property != "_id" && (!array_key_exists($property, $this->_docRequirements))) {
	...
	public function save($entierDocument = false, $safe = true)
	{
		...
		{
			$new_id = $operations["_id"];
			if ($new_id) {
				$cond = array("_id"=>$new_id);
			}else{
				$cond=$this->getCriteria();
			}
			$result = $this->_getMongoCollection(true)->update($cond, $operations, array('upsert' => true, 'safe' => $safe));
