...
class IdCode_Verify{
    ...
    public function setBgColor($color) {
        if (is_array($color) && is_numeric($color['r']) && 
            is_numeric($color['g']) && is_numeric($color['b']) && 
    ...
    public function setFont($font) {
        if (is_array($font)){
            ...
            if (!is_numeric($font['left']) || $font['left']<0 || 
            ...
            if (!file_exists($font['file'])) {
                $font['file'] = dirname(__FILE__).'/arial.ttf';
            }
            $this->font = $font;
        } else{
			...
			$this->font['left'] = $this->image['width'] - 
                                  ($this->font['size'] + $this->font['space']) 
	...
	function getVerifyCode(){
        $the_code = '';
        for ($i=0; $i<count($this->codeArr); $i++) {
            $the_code .= $this->codeArr[$i]['char'];
        }
		return $the_code;
	}
    ...
    public function paint($filename='') 
    {
        ...
        for ($i=0; $i<$num; $i++)  {
            $font_color = imagecolorallocate($im,
				$code[$i]['color']['r'], 
				$code[$i]['color']['g'], 
                $code[$i]['color']['b']);
            imagettftext($im, $this->font['size'], 
				$code[$i]['angle'], 
                $current_left,
				$current_top,
				$font_color, 
