<?php
define ('BACK_AUTH_NAME','backybkauthv10');
class Session_YnUser { 
	...
	private $login_url = 'https://www.yinbake.com/passport/signin';
	
	public $domain = 'www.yinbake.com';
	
	private function  __construct($login_url = null, $domain = null){
		$host = $_SERVER["HTTP_HOST"];
		if(empty($this->login_url)) {
			$host = $_SERVER["HTTP_HOST"];
			$this->login_url = "https://{$host}/user/signin";
		}
	...
	static public function instance($login_url = null, $domain = null){
		if(self::$obj)
			...
			self::$obj = new Session_YnUser($login_url, $domain);
		}
		return self::$obj;
	}
	...
	public function requireLogin($forward = '', $exit = true){
		if(! $this->isLogin()){
			if($forward === null){
				...
				if(empty($forward)){
					$forward = 'https://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
				}
