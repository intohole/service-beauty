<?php
define ('BACK_AUTH_NAME','wapynauthv10');
class Session_WapYnUser {
	...
	private $login_url = 'http://m.yinbake.com/passport/signin';

	public $domain = 'm.yinbake.com';

	public $host = 'm.yinbake.com';

	private function  __construct($login_url = null, $domain = null){
		$host = $_SERVER["HTTP_HOST"];
		if(empty($this->login_url)) {
			$host = $_SERVER["HTTP_HOST"];
			$this->login_url = "http://{$host}/passport/signin";
		}
	...
	static public function instance($login_url = null, $domain = null){
		if(self::$obj)
			...
			self::$obj = new Session_WapYnUser($login_url, $domain);
		}
		return self::$obj;
	}
	 ...
	 *Session_MaodouUser @param unknown_type $exit
	 */
	public function requireLogin($forward = '', $exit = true){
		if(! $this->isLogin()){
			if($forward === null){
				...
				if(empty($forward)){
					$requestUri = str_replace('/m/', '', $_SERVER['REQUEST_URI'] );
					//$forward = 'http://'.$this->host.$_SERVER['REQUEST_URI'];
					$forward = 'http://'.$this->host.$requestUri;
				}
