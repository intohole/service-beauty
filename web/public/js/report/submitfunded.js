...
btnSubmit.on('click', function(e){
	...
	var money = $('input[name="tail_money"]').val().trim();
	if(!money){
		pop.error('请填写申贷金额');
		btnSubmit.removeClass('disabled');
		return false;
	...
	var original=$("input[name=original_bank]").val().trim();
	if(!original){
		pop.error("请填写原银行名称");
		btnSubmit.removeClass("disabled");
		return false;
	}
	var payment=$("input[name=balance_payment]").val().trim();
	if(!payment){
		pop.error("请填写原银行尾款");
		btnSubmit.removeClass("disabled");
		return false;
...
function submitReport(){
	form.attr('action','/report/submitFundedAjax').attr('method','post').submit();
}
