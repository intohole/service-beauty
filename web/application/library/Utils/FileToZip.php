...
class Utils_FileToZip{
     ...
     * 压缩文件(zip格式)
     */
    public function tozip($items,$zip_name){ 
        $zip=new ZipArchive();
        ...
        if (!file_exists($zipname)){
            ...
            $zip->close();
            $dw=new Utils_Download($zipname.'.zip',$this->savepath,$zip_name); //下载文件
            $dw->getfiles();
            unlink($this->savepath.$zipname.'.zip');//下载完成后要进行删除   
			//print_r($this->currentdir);exit;
            $this->deleteDir($this->currentdir); //下载完成后要删除源文件夹  
        }
    }
...
class Utils_Download{
    ...
    protected $savepath;
    protected $zip_name;
    public function __construct($filename,$savepath,$zip_name){
        $this->_filename=$filename;
        $this->_filepath=$savepath.$filename;
        $this->_zip_name=$zip_name;
    }
    //获取文件名
    ...
    public function getfiles(){
        ...
        if (file_exists($this->_filepath)){
            ...
            //这里对客户端的弹出对话框，对应的文件名
            Header("Content-Disposition: attachment; filename=".$this->_zip_name.'.zip');
            //修改之前，一次性将数据传输给客户端
