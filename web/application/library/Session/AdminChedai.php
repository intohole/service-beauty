<?php
define ('BACK_AUTH_NAME','backfkchedaiauthv10');
class Session_AdminChedai {
	...
	private function  __construct($login_url = null, $domain = null){
		...
		$this->login_url =  'http://'.Yaf_Application::app()->getConfig()->website->host.'/chedai/login';
		$this->domain = Yaf_Application::app()->getConfig()->website->host;
		
		if(empty($this->login_url)) {
			$host = $_SERVER["HTTP_HOST"];
			$this->login_url = "http://{$host}/chedai/login";
		}
	...
	static public function instance(){
		if(self::$obj)
			...
			self::$obj = new Session_AdminChedai();
		}
		return self::$obj;
	}
	...
	public function requireLogin($forward = '', $exit = true){
		if(! $this->isLogin()){
			if($forward === null){
                ...
                //var_dump("location: ". $this->login_url . "?forward=$forward");exit;
				header("location: ". $this->login_url);
			}
			if($exit){
				exit;
