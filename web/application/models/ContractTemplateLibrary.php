 ...
 * @Desc: 合同模板模型
******************************/
class ContractTemplateLibraryModel extends TK_M {
    
    protected $tableName = 'xmcd_contract_template_library';
    ...
    public function getTemplateList($page,$pageSize,$where=null){
        ...
        $fields = "xmcd_contract_template_library.*";
        
        //不查询已删除的数据
        $this->where("xmcd_contract_template_library.status !=4");
        ...
        $list = $select->order('id asc')->select();
        
        foreach ($list as $key => $val) {
            //模板是否已制作过
            if($val['status'] == 1){
                $list[$key]['edit'] = "<a href=/contract/templateLibMake?id=" .$val['id']. ">重新编辑</a>";
            }
            else{
                $list[$key]['edit'] = "<a href=/contract/templateLibMake?id=" .$val['id']. ">添加</a>";
            }
...
// echo "<pre><meta charset='utf-8'>";var_dump($this->getLastSql());exit;
    ...
    public function getList(){
        $res = $this->field("id, name")->where(array('status'=>1))->select();
        return $res;
    }  

    public function gets($ids) {
        return $this->where(array('id'=>array('in', $ids), 'status'=>1))->select();
    }
}