...
<div id="main-container" class="container" style="position:relative">
 ...
 <div class="wrap" style="clear:both; margin:30px auto;">
	<section class="tab-content" id="contents">
      ...
      <h3 style=" float:left;margin-bottom:15px;"><img style="vertical-align:middle; height:30px" src="/Public/Home/images/weixin.png"/> 我创建的公众号</h3>
...
<a href="<?php echo U('Home/Public/check_res',array('id'=>$data[id]));?>" target="_self">自动检测</a>&nbsp;&nbsp;&nbsp;                
                <a href="<?php echo U('Home/PublicLink/lists',array('mp_id'=>$data[id]));?>" target="_self">管理员配置</a>&nbsp;&nbsp;&nbsp;
                <?php if(!C('PUBLIC_BIND') || !$data['is_bind']) { ?><a href="<?php echo U('step_0',array('id'=>$data[id]));?>" target="_self">编辑</a>&nbsp;&nbsp;&nbsp;<?php } ?>
                <a href="javascript:;" onclick="do_del('<?php echo ($data["id"]); ?>','<?php echo ($data["public_name"]); ?>')" >取消绑定</a> </td>
      ...
      -->
      <?php if(!empty($list_data[0])): ?><h3 style="margin:15px 0;"><img style="vertical-align:middle; height:25px" src="/Public/Home/images/user_icon.png"/> 我加入的公众号</h3>
      <div class="data-table" style="margin:0">
        <div class="table-striped">
          <table cellspacing="1">
            ...
            <?php if(is_array($list_data[0])): $i = 0; $__LIST__ = $list_data[0];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td><?php echo ($data["token"]); ?></td>
                <td><?php echo ($data["count"]); ?></td>
                <td><a href="<?php echo U('Home/Index/main',array('publicid'=>$data[id]));?>" target="_self">进入管理</a></td>
              </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            </tbody>
          </table>
        </div><?php endif; ?>
