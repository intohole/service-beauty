 ...
 */
    ...
    class {$data['info']['name']}Addon extends Plugin{
	...
	public function install() {
		\$install_sql = './Plugins/{$data['info']['name']}/install.sql';
	...
	public function uninstall() {
		\$uninstall_sql = './Plugins/{$data['info']['name']}/uninstall.sql';
...
str;
	...
	public function checkForm() {
		...
		if (! $data ['info'] ['name'])
			$this->error ( '插件标识必须' );
			// 检测插件名是否合法
		$addons_dir = ONETHINK_PLUGIN_PATH;
...
str;
	...
	public function edit($name, $id = 0) {
		...
		$this->assign ( 'title', $addon->info ['title'] );
		if (isset ( $model )) {
			$addonModel = D ( "Plugins://{$name}/{$model}" );
			if (! $addonModel)
				$this->error ( '模型无法实列化' );
