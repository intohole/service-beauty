...
class Shuipf extends TagLib {
     ...
     * 标签：<get></get>
     * 作用：特殊标签，SQL查询标签
     * 用法示例：<get sql="SELECT * FROM lvyecms_article  WHERE status=99 ORDER BY inputtime DESC" page="$page" num="5"> .. HTML ..</get>
     * 参数说明：
    ...
    public function _get($tag, $content) {
        ...
        $tag['sql'] = $sql = str_replace(array("think_", "lvyecms_"), C("DB_PREFIX"), strtolower($tag['sql']));
        //数据源
...
//                    $parseStr .= ' $get_db->db(1,"' . $db . '"); ';
                ...
                if ($_sql = preg_replace('/select([^from].*)from/i', "SELECT COUNT(*) as count FROM ", $tag['sql'])) {
                    ...
                    if (substr(trim($sql), 0, 1) == '$') {
                        $parseStr .= ' $sql = str_replace(array("think_", "lvyecms_"), C("DB_PREFIX"),' . $sql . ');';
