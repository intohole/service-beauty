...
class ModelModel extends Model {
    ...
    const membershipModelSql = 'Data/Sql/lvyecms_member.sql'; //会员模型
    ...
    public function AddModelMember($tableName, $modelId) {
        ...
        //表前缀，表名，模型id替换
        $sqlSplit = str_replace(array('@lvyecms@', '@zhubiao@', '@modelid@'), array($dbPrefix, $tableName, $modelId), $membershipModelSql);
    ...
    public function sql_split($sql) {
        ...
        if (C("DB_PREFIX") != "lvyecms_") {
            $sql = str_replace("lvyecms_", C("DB_PREFIX"), $sql);
        }
        $sql = str_replace("\r", "\n", $sql);
        $ret = array();
        $num = 0;
