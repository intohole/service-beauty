...
class Content {
    ...
    public function lists($data) {
        ...
        //是否经过ContentOutput处理
        if ($data['output']) {
            LvyeCMS()->ContentOutput->setModelid($this->modelid);
        }
        //把数据组合成以id为下标的数组集合
        if ($dataList) {
            $return = array();
            foreach ($dataList as $r) {
                ...
                if ($data['output']) {
                    ...
                    $return[$r['id']] = LvyeCMS()->ContentOutput->get($return[$r['id']]);
    ...
    public function hits($data) {
        ...
        foreach ($dataList as $r) {
            ...
            if ($data['output']) {
                ...
                $array[$r['id']] = LvyeCMS()->ContentOutput->get($array[$r['id']]);
        ...
        if ($data['relation']) {
            ...
            foreach ($_key_array as $r) {
                ...
                if ($data['output']) {
                    ...
                    $key_array[$key] = LvyeCMS()->ContentOutput->get($key_array[$key]);
        ...
        if ($data['keywords'] && $limit > $number) {//根据关键字的相关文章
            ...
            foreach ($keywords_arr as $_k) {
                ...
                foreach ($_r as $rs) {
                    ...
                    if ($data['output']) {
                        $_original = $r[$rs['id']];
                        $r[$rs['id']] = LvyeCMS()->ContentOutput->get($r[$rs['id']]);
