...
class IndexController extends Controller {
    ...
    public function step_2() {
        ...
        $folder = array(
            '/',
            '/d/',
            '/lvyecms/Application/Install/',
            '/lvyecms/Common/Conf/',
            '/lvyecms/Common/Conf/addition.php',
        );
        $dir = new \Dir();
        $folderInfo = array();
    ...
    public function mysql() {
        ...
        //读取数据文件
        $sqldata = file_get_contents(MODULE_PATH . 'Data/lvyecms.sql');
        //读取测试数据
        if ($testdata) {
            $sqldataDemo = file_get_contents(MODULE_PATH . 'Data/lvyecms_demo.sql');
            $sqldata = $sqldata . "\r\n" . $sqldataDemo;
        } else {
            //不加测试数据的时候，删除d目录的文件
            try {
